...
def initdb(drop):
    ...
    在使用命令flask initdb 时可以选择使用或不使用--drop，作用和action='store_true'类似
    '''
    if drop:
        db.drop_all()
    db.create_all()
...
def forge():
    ...
    movies = [
        ...
        {'title': 'Fudan University', 'year': '1905'},
    ]
...
def settings():
    if request.method == 'POST':
        name = request.form['name'] #  这个修改的是一个网页内用户的昵称，不是登录的用户名
...
def page_not_found404(e):  # 接受异常对象作为参数
    return render_template('404.html'), 404  # 返回模板和状态码


@app.errorhandler(400)  # 传入要处理的错误代码
def page_not_found400(e):  # 接受异常对象作为参数
    return render_template('404.html'), 400  # 返回模板和状态码


@app.errorhandler(500)  # 传入要处理的错误代码
def page_not_found500(e):  # 接受异常对象作为参数
    return render_template('404.html'), 500  # 返回模板和状态码
